...
function fetchingData () {
  cLog('Fetching votes...')
  let statsSet = false
  let fetchingDataFromServertimer
  //fetchingData();
  function fetchingDataFromServer () {
    if (getVideoId() == null) return
    try {
      fetch(
        ...
        response.json().then(json => {
          if (json && !('traceId' in response) && !statsSet) {
            ...
            if (
              document.querySelector('[aria-label="Dislike this video"] > span')
                .textContent != dislikesvalue
            ) {
              setState()
            }
          }
    ...
    clearInterval(fetchingDataFromServertimer)
  }
  fetchingDataFromServertimer = setInterval(fetchingDataFromServer, 100)
}
  ...
  function checkForJS_Finish () {
    ...
    if (isShorts() || (getButtons()?.offsetParent && isVideoLoaded())) {
      ...
      try {
        ...
        buttons.children[0].children[1].addEventListener(
          'click',
          dislikeClicked
        )
        buttons.children[0].children[0].addEventListener(
          'touchstart',
          likeClicked
        )
        buttons.children[0].children[1].addEventListener(
...
;(function () {
  'use strict'
  fetchingData()
  window.addEventListener('yt-navigate-start', fetchingData, true)
